...
//#if _MSC_VER < 1800
inline bool std_signbit(double num) { return _copysign(1.0, num) < 0; }
//#endif

  LogVal() : s_(), v_(LOGVAL_LOG0) {}
  LogVal(double x) : s_(std_signbit(x)), v_(s_ ? std::log(-x) : std::log(x)) {}
  const Self& operator=(double x) { s_ = std_signbit(x); v_ = s_ ? std::log(-x) : std::log(x); return *this; }
